...
from polls.serializers import AccountSerializer
from polls.models import AccountModel
...
class NotificationManager:
    ...
    def add(self, consumer: NoticeConsumer):
        ...
        unsend_notices = NotificationModel.objects.filter(user=consumer.user)
        ...
        for friend in consumer.user.profilemodel.get_friends():
            self.notify_user(friend.user, {'type': 'online',
                                           'user': AccountSerializer(consumer.user.profilemodel).data})
    ...
    def notify_user(self, user: User, data: dict):
        ...
        if consumer:
            ...
            NotificationModel(user=user, data=data_str).save()

    def notify_friend_request(self, user: User, friend: AccountModel):
        self.notify_user(user, {'type': 'friend_request', 'author': AccountSerializer(friend).data})

    def notify_friend_request_canceled(self, user: User, friend: AccountModel):
        self.notify_user(user, {'type': 'friend_request_canceled', 'author': AccountSerializer(friend).data})

    def notify_new_friend(self, user: User, friend: AccountModel):
        serialized_data = AccountSerializer(friend).data
        ...
        self.notify_user(friend.user, {'type': status, 'user': AccountSerializer(user.profilemodel).data})

    def notify_friend_removed(self, user: User, friend: AccountModel):
        self.notify_user(user, {'type': 'friend_removed', 'friend': AccountSerializer(friend).data})
...
class NoticeConsumer(WebsocketConsumer):
    def connect(self):
        ...
        self.accept()
        notification_manager.add(self)

    def disconnect(self, code):
        if self.scope['user'].is_authenticated:
            notification_manager.remove(self)
